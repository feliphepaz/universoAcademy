...
const Account = () => {
    ...
    const [videoTitle, setVideoTitle] = React.useState('Como instalar o Adobe XD');
        ...
        <section className='account'>
            <div className='container'>
                <div className='video-main'>
                    <h1>
                        <legend>Aula {videoId} - Módulo {modulo}</legend>
                        {videoTitle}
                    </h1>
                    <iframe width="100%" height="640" src={linkEmbed} title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
                </div>
                <nav className='modulo'>
                    <h2>Módulo 1 - Introdução</h2>
                    <ul>
                        {videosMod1.map((video) => (
                            <li className='video' key={video.id} onClick={() => { setVideoTitle(video.titulo); setVideoCode(video.codigo); setVideoId(video.id); setModulo(1); window.scrollTo(0,0)}}>
                ...
                <nav className='modulo'>
                    ...
                    <ul>
                        {videosMod2.map((video) => (
                            <li className='video' key={video.id} onClick={() => { setVideoTitle(video.titulo); setVideoCode(video.codigo); setVideoId(video.id); setModulo(2); window.scrollTo(0,0)}} >
                                <legend>Aula {video.id}</legend>
                                <img alt={video.titulo} src={getVideoCode(video.codigo)}></img>
                                <h3>{video.titulo}</h3>
                            </li>
